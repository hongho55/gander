...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        function getSlideSizeAndSetDefaultTextStyle(zip) {
            // app.xml is optional in valid PPTX packages (for example, files
            // exported by Google Slides). Missing metadata must not stop rendering.
            var app = readXmlFile(zip, "docProps/app.xml") || {};
            var appProperties = app["Properties"] || {};
            var app_verssion_str = appProperties["AppVersion"];
            app_verssion = parseInt(app_verssion_str, 10);
            if (isNaN(app_verssion)) app_verssion = 16;
            console.log("create by Office PowerPoint app verssion: ", app_verssion_str || "unknown")
