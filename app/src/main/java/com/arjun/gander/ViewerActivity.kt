...
class ViewerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (kind == FileKind.PPTX && PptxOpening.usesExternalApp(this)) {
            openPptxInAnotherApp(uri, mime)
            finish()
            return
        }
        when (kind) {
    ...
    private fun openPptxInAnotherApp(uri: Uri, suppliedMime: String?) {
        val intent = Intent(Intent.ACTION_VIEW)
            .setDataAndType(
                uri,
                suppliedMime ?: "application/vnd.openxmlformats-officedocument.presentationml.presentation"
            )
            .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            .putExtra(
                Intent.EXTRA_EXCLUDE_COMPONENTS,
                arrayOf(ComponentName(this, ViewerActivity::class.java))
            )
        val hasExternalHandler = packageManager.queryIntentActivities(intent, 0)
            .any { it.activityInfo.packageName != packageName }
        if (!hasExternalHandler) {
            Toast.makeText(this, R.string.pptx_external_apps_unavailable, Toast.LENGTH_SHORT).show()
            return
        }
        startActivity(Intent.createChooser(intent, getString(R.string.open_pptx_elsewhere)))
    }
